<template>
    <div class="Parents">
        <h2 class="parentsTitle">Родители</h2>
            <div class="parentsPhotos">
                <div class="husbandsParents">
                    <div class="parentPhotoBlock">
                        <img class="parentPhoto" src="../img/Parents/husbandsFather1.jpg"/>
                        <h4>Сергей Виталиевич Огурцов</h4>
                        <p class="parentsProfession">Сварщик VI разряда. Почётный работник ОАО "Воркутинский механический завод"</p>
                    </div>
                    <div class="parentPhotoBlock">
                        <img class="parentPhoto" src="../img/Parents/husbandsMother1.jpg"/>
                        <h4>Екатерина Андреевна Огурцова</h4>
                        <p class="parentsProfession">Швея высшей категории. Сотрудница ОАО "Воркутинская швейная фабрика"</p>
                    </div>
                </div>
                <div class="wifesParents">
                    <div class="parentPhotoBlock">
                        <img class="parentPhoto" src="../img/Parents/wifesFather1.jpg"/>
                        <h4>Олег Вениаминович Трубецкой</h4>
                        <p class="parentsProfession">Генеральный директор ООО «Газпром трансгаз Санкт-Петербург»</p>
                    </div>
                    <div class="parentPhotoBlock">
                        <img class="parentPhoto" src="../img/Parents/wifesMother1.jpg"/>
                        <h4>Марианна Владимировна Трубецкая</h4>
                        <p class="parentsProfession">Владелица сети цветочных салонов "Гортензия"</p>
                    </div>
                </div>
            </div>
        </div>
</template>


<script>
export default {
    name: 'my-parents'
}
</script>


<style scoped>

.parentsTitle {
    color: rgb(68, 68, 68);
    margin-top: 35px;
    margin-bottom: 35px;
    display: flex;
    justify-content: center;
    font-size: 22pt;
    font-family:'Circe Light', sans-serif !important;
    
}
.parentPhoto {
    border-radius: 40px;
    width: 90%;
    padding-top: 15px;
    padding-bottom: 20px;
    
}
.parentPhotoBlock {
    
    text-align: center;
    border-radius: 10px;

}
.parentsProfession {
    padding-bottom: 30px;
}
@media screen and (min-width: 500px) {
.parentsPhotos {
    display: flex;
    flex-direction: row;
}
.husbandsParents {
    display: flex;
    flex-direction: column;
    max-width: 50%;
}
.wifesParents {
    display: flex;
    flex-direction: column;
    max-width: 50%;
}
}
</style>